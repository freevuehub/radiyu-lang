...
const 콘솔 = (...값: (string | number)[]) => console.log('야옹', ...값)
...
const 라인_실행 = async (코드: string) => {
  if (/^\s+허어/g.test(코드)) {
    const [선언부, 타입] = 조각_유효성_검사(코드, /^\s+허어(.+)(.{2}!)$/g)
  ...
  if (/^\s+야옹/g.test(코드)) {
    const [선언부] = 조각_유효성_검사(코드, /^\s+야옹(.+)$/g)
    const 일반_선언부 = await 수식_계산(선언부)

    if (!isNaN(Number(일반_선언부)))
      return 콘솔(Number(일반_선언부))

    return 콘솔(변수_모음_체크(일반_선언부)['값'])
  }

  return ''
}

const 가자 = async (코드: string) => {
  const 라인들 = 코드.trim().split('\n')
...
await 가자(await 파일_유효성_검사(Deno.args[0]))
